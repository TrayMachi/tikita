...
export const OrderSaya = () => {
  const { user } = useAuth();
  const [orders, setOrders] = useState<OrderWithDetails<"seller">[]>([]);
  const [detailOrder, setDetailOrder] =
    useState<OrderWithDetails<"seller"> | null>(null);
  ...
  return (
    <View>
      ...
      <VStack space="lg">
        {detailOrder ? (
          <OrderDetail order={detailOrder} setDetailOrder={setDetailOrder} />
        ) : (
            ...
            return (
              ...
              >
                <Pressable
                  ...
                  accessibilityLabel={`Order for ${order.ticket.name}`}
